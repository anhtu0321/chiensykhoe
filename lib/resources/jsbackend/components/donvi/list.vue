    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr v-for="(list, index) in listData.data" :key="list.id" :class="list.id == idEdit? 'tractive':''">
                ...
                <td>{{list.ten_khoi}}</td>
                <td>
                    <router-link class="btn btn-primary btn-sm" :to="`/donvi/edit/${list.id}`" @click.native="loadDataById(list.id)">Sửa</router-link>
    ...
    methods:{
        ...
        deleteData(id){
            ...
            .then(value=>{
                ...
                if(value == 'delete'){
                    axios.get('/chiensykhoe/admin/deleteDonvi/'+id)
                    .then(res=>{
                        this.$emit('deleted');
                        swal('Xóa Thành công !');
                    })
